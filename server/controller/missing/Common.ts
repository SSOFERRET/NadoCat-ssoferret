...
export const getPosts = async (req: Request, res: Response, postData: IListData, missingId?: number) => {
  try {
    ...
    const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      let posts: any[] = [];
      let userIds: string[] = [];
      if (categoryId === CATEGORY.MISSINGS) [posts, userIds] = await getPostList(listData);
      else if (categoryId === CATEGORY.MISSING_REPORTS) [posts, userIds] = await getPostList(listData, missingId);
